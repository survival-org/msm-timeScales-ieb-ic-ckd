...
dir_figures <- "/nvmetmp/wis37138/msm_kidneyFunction/results/simulations/figures/ieb/"
repls <- 200
ncores <- 200
seed <- 11022022
  ...
  addProblem(
    ...
    fun  = function(job, data, dist_x1, dist_x2, beta_scenario, beta_vals) {
      wrapper_sim(
        ...
        dist_x1          = dist_x1,
        dist_x2          = dist_x2,
        beta_vals        = beta_vals,        # ← pass it through
...
cor_summary <- res_cor %>%
  ...
  summarise(
    ...
    mutate(rho_ci = map_chr(rho_ci, ~ paste0("(", .[1], ", ", .[2], ")")))

# plot results ----
problem_labs <- c(sim_timeScales = "time-scales DGP",
                  sim_stratified = "stratified DGP")

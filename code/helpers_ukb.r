...
create_eGFR15 <- function(pheno){ # requires columns id, eGFRcrea, date_at_exam
    eGFR15 <- pheno %>%
        group_by(id) %>%
...
add_status <- function(pheno){ # requires columns id, eGFRcrea, date_at_exam, date_at_exam_eskd
...
impute_events <- function(events, pheno){
    ...
    #    c) For those individuals in idx_candidates, exclude if their last pheno row has eGFRcrea > 15
    idx_exclude <- pheno_last %>% filter(id %in% idx_candidates & eGFRcrea >= 15) %>% pull(id)
